...
public class FallbackResourceStore<T>(List<IResourceStore<T>> resourceStores) : IResourceStore<T>
{
    ...
    public IEnumerable<string> List(string prefix = "") => resourceStores.SelectMany(s => s.List(prefix)).Distinct();


    public void Dispose() => resourceStores.ForEach(s => s.Dispose());
}
