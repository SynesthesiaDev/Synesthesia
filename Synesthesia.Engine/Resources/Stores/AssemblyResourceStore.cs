...
public class AssemblyResourceStore<T>(Assembly assembly, Dictionary<string, Func<Stream, T>> loaders) : IResourceStore<T>
{
    private readonly Dictionary<string, Func<Stream, T>> loaders = loaders.ToDictionary(k => k.Key.ToLowerInvariant().RemoveSuffix("."), k => k.Value);
